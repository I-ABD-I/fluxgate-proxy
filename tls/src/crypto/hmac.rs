...
impl Prf for PrfUsingHmac<'_> {
    fn for_key_exchange(&self, output: &mut [u8; 48], kx: Box<dyn ActiveKx>, peer_pub: &[u8], label: &[u8], seed: &[u8]) -> Result<(), Error> {
        prf(output, &*self.0.with_key(&kx.complete(peer_pub)?),label, seed);
        Ok(())
    }
